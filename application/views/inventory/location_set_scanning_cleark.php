  ...
  <?php if($continue==0){?>
    <div class="alert alert-danger">
      ...
      Last update of WMS inventory import is : <strong><?php echo $last_inventory_import?></strong>
		...
		<form method="post" id="form1" action="<?php echo base_url()?>index.php/inventory/dashboard/location_set_scanning_clearks">
        <input name="date"  id="date" value="<?php echo $date?>" type="hidden" />
      <input name="continue"  id="continue" value="0" type="hidden" /> </form>
</div>
   <?php }else{?> 
       <section class="content-header">
      <h1> Set Scanning Clearks for  count on <?php echo $date?></h1>
    </section>
